...
int main(int argc, char** argv) {
	...
	unsigned int num_nodes = 2000;
	// Maximal number of iterations performed in local search
	unsigned int max_iterations = 350;
	// filter ratio: how many moves filtered out
	float filter_ratio = 0.66f;
	...
	if (argc > 2)
	{
		float r = static_cast<float>(atof(argv[2]));
		if (r >= 0 && r <= 1)
			filter_ratio = r;
	}
	...
	std::cout << "filter out:         " << filter_ratio*100.0 << "%" << std::endl;
	...
	unsigned int gpu_num_iterations = GPU::local_search(gpu_solution, city_coordinates, max_iterations, filter_ratio);
