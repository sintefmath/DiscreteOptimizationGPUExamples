...
__global__ void evaluate_moves_kernel(unsigned int* solution_, const float* city_coordinates_, float* deltas_, unsigned int* moves_, 
	                                  unsigned int num_nodes_, unsigned int num_moves,
									  int *filter_) {
	...
	for (int i=tid; i < num_moves; i += grid_size) {
		Move move = generate_move(filter_[i], num_nodes_, solution_);
...
unsigned int local_search(std::vector<unsigned int>& solution, const std::vector<float> &city_coordinates, const unsigned int max_iterations, 
	...
	err = cudaMalloc(&filter_compacted_gpu, num_moves*sizeof(int));
	if (err != cudaSuccess) {
		std::cout << "Could not allocate GPU memory for filter" << std::endl;
		return num_iterations;
	}
	...
	for (;;) {
		...
		thrust::device_ptr<int> last = thrust::copy_if(thrust::counting_iterator<int>(0), thrust::counting_iterator<int>(num_moves), thrust::device_ptr<int>(filter_compacted_gpu), Filter(filter_gpu));
		int number_of_filtered_moves = last-thrust::device_ptr<int>(filter_compacted_gpu);
		evaluate_moves_kernel<<<evaluate_grid, evaluate_block>>>(solution_gpu, coords_gpu, deltas_gpu, moves_gpu, num_nodes, number_of_filtered_moves, filter_compacted_gpu);
